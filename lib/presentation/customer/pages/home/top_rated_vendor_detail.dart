...
class VendorDetail extends StatefulWidget {
  final String id;
  final String name;
  const VendorDetail({Key? key, required this.id, required this.name}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ShopController>(
      init: ShopController(),
        builder: (controller){
      return Scaffold(
          floatingActionButton: GetBuilder<ShopController>(
            init: ShopController(),
                  ...
                  child: Stack(alignment: Alignment(1.4, -1.5),
                    children: [
                      ...
                      Container(             // This is your Badge
                        child: Center(
                          child: Text("${_rsController.totalItemInCart}", style: TextStyle(color: Colors.white)),
                        ),
          ...
          NestedScrollView(controller: scrollController,
            physics: const BouncingScrollPhysics(), headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
            return [
              SliverAppBar(scrolledUnderElevation: 0.0,
                expandedHeight: MediaQuery.of(context).size.height / 4, floating: true, automaticallyImplyLeading: false,
                backgroundColor: white, pinned: true, elevation: 0.0,
                flexibleSpace: FlexibleSpaceBar(
                  background: Container(
                      height: MediaQuery.of(context).size.height / 4, width: double.maxFinite,
                      decoration: BoxDecoration(
                          image: DecorationImage(image:
                          NetworkImage(controller.shopResponseModel?.data?.coverImage ?? imagePlaceHolder), fit: BoxFit.cover))
                  ),
                ),
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.start,
                      children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text(controller.shopResponseModel?.data?.name ?? "", overflow: TextOverflow.fade,
                        ...
                        Align(alignment: Alignment.centerLeft,
                            child: DescriptionTextWidget(text:controller.shopResponseModel!.data!.biography ?? "",)),
                      ],
                    )
                ),
              ),
